...
const fetchReviews = async () => {
  try {
    const res = await fetch('http://https://mochi-mevn.onrender.com/api/admin/reviews', {
      credentials: 'include'
    })
...
const confirmDelete = async () => {
  try {
    const res = await fetch(`http://https://mochi-mevn.onrender.com/api/admin/reviews/${reviewToDelete.value._id}`, {
      method: 'DELETE',
      credentials: 'include'
    })
    const data = await res.json()
